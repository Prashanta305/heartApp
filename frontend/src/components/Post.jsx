...
const Post = ({ post }) => {
  ...
  return (
    <div className='my-8 w-full max-w-sm max-auto'>
      <div className='flex items-center justify-between'>
        ...
        <Dialog>
          ...
          <DialogContent className=" flex flex-col items-center text-sm text-center">
            {
              post?.author?._id !== user?._id && <Button variant='ghost' className="cursor-pointer w-fit text-[#ED4956] font-bold">unfollo</Button>
            }
      ...
      <p>
        <span font-medium block mb-2>{post.author?.username} </span>
        {post.caption}
      </p>
      {
        comment.length > 0 && (
      ...
      <div className='flex items-center justify-between'>
        <input type="text" value={text} placeholder='Add a comment...' onChange={changeEventHandler} className='outline-none text-sm w-full' />
        {
